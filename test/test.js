...
describe.only("Checking ICO contract", function () {
    ...
    describe("Transactions", function () {
        ...
        describe("Check! function transfer(_to, _value)", function () {
            describe("before the sale", function () {
                it("Check! (regular user).transfer(...) => Permission denied", async function () {
                    ...
                    await expect(ico.connect(addr1).transfer(addr2.address, 50)).to.be.revertedWith("Permission denied");
                });
        ...
        describe("Check! buy token", function () {
            describe("before the sale", function () {
                ...
                it("Check! (regular user) send 1 eth => Sale is not available at the moment.", async function () {
                    ...
                    await expect(addr1.sendTransaction(tx_buy))
                    .to.be.revertedWith("Sale is not available at the moment.");
                });
		...
		it("Should fail if sender doesn’t have enough tokens => ERC20: transfer amount exceeds balance", async function () {
            ...
            await expect(ico.connect(addr1).transfer(addr2.address, 51))
		...
		it("function withdrawBalance(_user)", async function () {
            ...
            expect(await ethers.provider.getBalance(ico.address)).to.equal(await ethers.utils.parseEther("2"));
            ...
            expect(await ethers.provider.getBalance(ico.address)).to.equal(await ethers.utils.parseEther("0"));

            // Теперь у addr2 должно быть 10002
            expect(await ethers.provider.getBalance(addr2.address)).to.equal(await ethers.utils.parseEther("10002"));
		});
